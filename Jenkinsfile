pipeline {
    ...
    stages {
        stage('Check') {
            steps {
                git branch: 'develop',credentialsId:'shinyoung-go', url:'https://github.com/Spharos-final-project-WOOYANO/Eureka-Server'
            }
        }
        stage('Build'){
            steps{
                script {
                    ...
                    def fileList = sh(script: "find /* -not -name ${excludedFile}", returnStdout: true).trim()
